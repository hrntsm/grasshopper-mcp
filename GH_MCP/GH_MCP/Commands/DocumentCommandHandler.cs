        ...
        public static object ClearDocument(Command command)
        {
            ...
            RhinoApp.InvokeOnUiThread(new Action(() =>
            {
                try
                {
                    ...
                    // Filter out essential components (keep those used for communicating with Claude Desktop)
                    // Here we can identify essential components by GUID, name, or type
                    var essentialComponents = objectsToRemove.Where(obj =>
                        ...
                        // Additional check for component names
                        obj.NickName.Contains("Toggle") ||
                        obj.NickName.Contains("Status") ||
                        obj.NickName.Contains("Panel")
                    ).ToList();

                    // Remove essential components from the list to be deleted
